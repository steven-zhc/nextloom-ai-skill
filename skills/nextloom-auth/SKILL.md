...
Manage Nextloom AI authentication via the `nextloom` CLI. Nextloom uses browser-based PKCE OAuth with a local loopback callback — no passwords to manage.
...
**CI / headless environments**: Set `NEXTLOOM_ACCESS_TOKEN` and `NEXTLOOM_REFRESH_TOKEN` environment variables instead. Get these tokens by running `nextloom auth login` on a machine with a browser, then copying the tokens from `~/.config/nextloom/auth-prod.json`.
...
Removes stored tokens. You'll need to `nextloom auth login` again to use any other commands.

### Switch Environment

```bash
nextloom auth whoami --env dev    # Check dev environment
nextloom auth login --env dev     # Login to dev
```
...
| Error | What to do |
|-------|-----------|
| `exit code 4` | Not authenticated. Run `nextloom auth login`. |
